...
const menuList: MenuItem[] = [
  ...
  {key: 'shop', name:'Lowest Price', title: '최저가격', icon: <ShoppingCartOutlined/>,
    subMenus: [
      {key: 'shop-1', link: 'shop/lowest/category', name:'Category', title: '카테고리별 최저가격', icon: <PaperClipOutlined/>},
      {key: 'shop-2', link: 'shop/minmax/category', name:'Item', title: '단일 카테고리 최저가격/최고가격', icon: <DatabaseOutlined/>},
      {key: 'shop-3', link:'shop/lowest/brand', name:'Brand', title: '단일 브랜드 최저가격', icon: <CheckSquareOutlined/>},
    ]
  },
]
...
const App: FC = () => {
  ...
  const menuClick = (e: MenuInfo) => {
    const selected = menuList.flatMap(item =>
      item.subMenus ? [item, ...item.subMenus] : [item]
    ).find((m) => m.key == e.key) || menuList[0];
    setMenu(selected)
  }
                ...
                return item.subMenus ?
                      ...
                      {item.subMenus.map((sub) => {
                        return (
                          <Menu.Item key={sub.key} icon={sub.icon}>
                            <Link to={sub.link || sub.key}>{sub.name}</Link>
                          </Menu.Item>
                        )})
                      }
                    </Menu.SubMenu>
                  ...
                  : (
                    <Menu.Item key={item.key} icon={item.icon}>
                      <Link to={item.link||item.key}>{item.name}</Link>
                    </Menu.Item>
                )
              })}
            </Menu>
          ...
          <Layout>
            ...
            >
              <Routes>
                ...
                <Route path="/shop/lowest/category" element={ <CategoryLowestPrice /> } />
                <Route path="/shop/minmax/category" element={ <CategoryMinMaxPrice /> } />
                <Route path="/shop/lowest/brand" element={ <BrandLowestPrice /> } />
              </Routes>
            </Content>
          </Layout>
        </Layout>
